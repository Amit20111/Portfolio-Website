          ...
          {categories.map((cat) => (
            <button
              ...
              style={{
                ...
                background: filter === cat ? t.accent : t.surface,
            ...
            <Card t={t} onClick={() => navigate(`/projects/${p.id}`)}>
              ...
              >
                <div>
                  ...
                  <p style={{ fontSize: 13, color: t.accent }}>{p.subtitle}</p>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <div style={{ fontSize: 12, color: t.muted }}>{p.year}</div>
                  <div style={{ fontSize: 11, color: t.muted, marginTop: 2 }}>{p.category}</div>
                </div>
              </div>

              <p style={{ color: t.muted, fontSize: 14, lineHeight: 1.7, marginBottom: 14 }}>{p.desc}</p>
              ...
              >
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {p.tags.slice(0, 4).map((tag) => <Tag key={tag} label={tag} t={t} />)}
                  {p.tags.length > 4 && <Tag label={`+${p.tags.length - 4}`} t={t} />}
                </div>
                <span style={{ fontSize: 13, color: t.accent, fontWeight: 500, flexShrink: 0 }}>
                  View Details →
                </span>
              </div>
            </Card>
